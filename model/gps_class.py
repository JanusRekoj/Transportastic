...
class GPSVis(object):
    ...
    """
    def __init__(self, data_path, map_path, img, points):
        """
    ...
    def plot_map_new(self, output='save', save_as='resultMap_new.png'):
        """
        Method for plotting the map. You can choose to save it in file or to plot it.
        :param output: Type 'plot' to show the map or 'save' to save it.
        :param save_as: Name and type of the resulting image.
    ...
    def create_image(self, data, stations, str_line_name, color, width=2):
        """
        ...
        draw.text(random.choice(img_points), str_line_name, fill=color, width=width)

        station_data = tuple(zip(stations['latitude'].values, stations['longitude'].values))
        for station in station_data:
            x1, y1 = self.scale_to_img(station, (self.result_image.size[0], self.result_image.size[1]))
            draw.rectangle([x1, y1, x1 + 5, y1 + 5], outline=color, fill='black')
